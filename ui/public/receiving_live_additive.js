...
function addTicker(msg, opts = {}) {
  ...
  // -------------------- Exports (CSV + supplier summary print-to-PDF) --------------------
  ...
  function openSupplierSummaryPrint(rows, weekStartISO) {
    const bySupplier = new Map();
    for (const r of rows || []) {
      const s = String(r.supplier_name || '').trim() || '(Unknown Supplier)';
    ...
    const cutoffLocal = fmtLocalFromUtc(businessCutoffUtcISO(weekStartISO));
    ...
    const supplierPages = suppliers.map(supplier => {
      ...
      const cartons = rs.reduce((a, x) => a + (Number(x.cartons_in) || 0), 0);
      const damaged = rs.reduce((a, x) => a + (Number(x.damaged) || 0), 0);
      const nonc = rs.reduce((a, x) => a + (Number(x.non_compliant) || 0), 0);
      const repl = rs.reduce((a, x) => a + (Number(x.replaced) || 0), 0);

      const rowsHtml = rs.map(x => `
        <tr>
          <td>${esc(x.po_number)}</td>
          <td>${esc(x.facility_name)}</td>
          <td class="right">${Number(x.cartons_in) || 0}</td>
          <td class="right">${Number(x.damaged) || 0}</td>
          <td class="right">${Number(x.non_compliant) || 0}</td>
          <td class="right">${Number(x.replaced) || 0}</td>
    ...
    const html = `
      ...
      <html><head><meta charset="utf-8" />
        <title>Receiving Supplier Summary — ${esc(weekStartISO)}</title>
        ${pageCss}
      </head>
      <body>
        <h1>Receiving — Supplier Summary</h1>
        <div class="meta">Week: ${esc(weekStartISO)} • Business cutoff (local): ${esc(cutoffLocal || '')}</div>
        ${supplierPages}
...
<div class="flex items-center gap-2 flex-wrap">
  ...
  <button id="recv-dl-week-csv" class="cmd cmd--ghost" title="Download week receiving as CSV (Excel)">
    Download Week (CSV)
  </button>

  <button id="recv-dl-supplier-pdf" class="cmd cmd--ghost" title="Open supplier summary for printing to PDF">
...
function computeSummaryAll(planRows, receivingRows) {
  ...
  function renderSummary(sum) {
    ...
    if (healthEl) {
      ...
      // minimalist contrast (no motion)
      healthEl.style.borderWidth = '1px';
      healthEl.style.borderStyle = 'solid';
      if (health === 'Delayed') {
        healthEl.style.borderColor = '#fecaca'; // red-200
        healthEl.style.background = '#fef2f2'; // red-50
        healthEl.style.color = '#b91c1c'; // red-700
      } else if (health === 'On-track') {
        healthEl.style.borderColor = '#bbf7d0'; // green-200
        healthEl.style.background = '#f0fdf4'; // green-50
        healthEl.style.color = '#15803d'; // green-700
      } else {
        healthEl.style.borderColor = '#e5e7eb'; // gray-200
        healthEl.style.background = '#f9fafb'; // gray-50
        healthEl.style.color = '#6b7280'; // gray-500
      }
    }

    // emphasize delayed count when non-zero
    const delayedEl = document.getElementById('recv-sum-delayed');
    if (delayedEl) delayedEl.style.color = (Number(sum.delayed) || 0) > 0 ? '#b91c1c' : '';
  }
...
// Download buttons (week-level, all suppliers)
...
if (btnCsv) {
  btnCsv.onclick = () => {
    ...
    if (!ws2) return;
    const rows = buildWeekExportRows(M.planRows, M.receivingRows, ws2);
    const header = [
    ...
    downloadTextFile(`receiving_week_${ws2}.csv`, lines.join('\n'), 'text/csv');
  };
}
...
if (btnPdf) {
  btnPdf.onclick = () => {
    ...
    if (!ws2) return;
    const rows = buildWeekExportRows(M.planRows, M.receivingRows, ws2);
    openSupplierSummaryPrint(rows, ws2);
  };
}
