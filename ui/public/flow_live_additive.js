    ...
    if (sel.node === 'lastmile') {
      ...
      for (let i = 0; i < weekContainers.length; i++) {
        ...
        contRows.push({
          ...
          vessel,
          delivered_at: String((rcp && (rcp.delivered_at || rcp.delivery_at || rcp.delivery_iso)) || '').trim(),
          delivery_local: String((rcp && (rcp.delivery_local || ((rcp.delivered_at || rcp.delivery_at || rcp.delivery_iso) ? String((rcp.delivered_at || rcp.delivery_at || rcp.delivery_iso)).slice(0,16) : ''))) || '').trim(),
          pod_received: !!((rcp && rcp.pod_received) || (rcp && (rcp.delivered_at || rcp.delivery_at || rcp.delivery_iso)) || false),
...
// Status
      ...
      const insights = [
        ...
        contRows.length ? `${contRows.filter(r => r.delivered_at).length}/${contRows.length} containers delivered.` : 'Add containers under Transit & Clearing to start tracking Last Mile.',
      ];
      ...
      const kpis = `
        <div class="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <div class="rounded-lg border p-2">
            <div class="text-[11px] text-gray-500">% Delivered</div>
            <div class="text-sm font-semibold">${contRows.length ? Math.round(100 * contRows.filter(r => r.delivered_at).length / contRows.length) : 0}%</div>
          </div>
        </div></div>
      `;
      ...
      const rows = contRows.map(r => {
        ...
        const deliveredCell = delivered
          ? escapeHtml(String(r.delivery_local || r.delivered_at).replace('T',' ').replace('Z',''))
          : `<button class="px-2 py-1 rounded-lg text-xs border bg-emerald-50 hover:bg-emerald-100 whitespace-nowrap"
              data-lm-deliver="1"
              data-ws="${escapeAttr(ws)}"
              data-cont="${escapeAttr(r.key)}"
              data-uid="${escapeAttr(r.uid)}">Delivered now</button>`;
  ...
  function wireLastMileDetail(ws, selectedKey) {
    ...
    // container row clicks (kept for future drill-down; safe even if editor is hidden)
    ...
    // Row-level "Delivered now" buttons
    detail.querySelectorAll('[data-lm-deliver]').forEach(btn => {
      if (btn.dataset.bound) return;
      btn.dataset.bound = '1';
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        ...
        const contKey = String(btn.getAttribute('data-cont') || selectedKey || '').trim();
        const uid = String(btn.getAttribute('data-uid') || '').trim();

        if (!uid) return;

        const nowIso = new Date().toISOString();
        const nowLocal = nowLocalDT();

        const receipts = loadLastMileReceipts(wsNow);
        receipts[uid] = {
          ...(receipts[uid] || {}),
          delivered_at: nowIso,
          delivery_local: nowLocal,
          pod_received: true,
          _updatedAt: nowIso,
        };
        saveLastMileReceipts(wsNow, receipts);

        // Keep selection and force a deterministic re-render for the same week
        if (contKey) UI.selection = { node: 'lastmile', sub: contKey };
