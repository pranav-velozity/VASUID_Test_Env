...
// Status
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mt-3">
          <div class="rounded-xl border p-3">
            ...
            <div class="flex items-center justify-between mt-3">
              ...
              <!-- IMPORTANT: do NOT use data-lane on this button; data-lane is reserved for lane selection targets -->
  ...
  function wireIntlDetail(ws, selectedKey) {
    ...
    // Lane row clicks (selection)
    // NOTE: data-lane is reserved for lane selection targets in the lane list table.
    // Do not attach this handler to action buttons inside the editor (e.g., Save lane).
    detail.querySelectorAll('[data-lane]').forEach(btn => {
      if (btn.dataset.bound) return;
      btn.dataset.bound = '1';
      btn.addEventListener('click', (e) => {
        // Ignore clicks that originate from action buttons/inputs.
        if (btn.id === 'flow-intl-save') return;
    ...
    if (saveBtn && !saveBtn.dataset.bound) {
      ...
      saveBtn.addEventListener('click', () => {
        ...
        // Save button uses data-lane-key to avoid conflicting with lane selection handler.
