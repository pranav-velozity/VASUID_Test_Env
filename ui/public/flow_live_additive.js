...
<div class="rounded-xl border p-3">
  ...
  ${vasMixHtml(vas)}
    ...
    if (sel.node === 'lastmile') {
      ...
      const rows = contRows.map(r => {
        ...
        const deliveredCell = r.delivery_at
          ? `<span class="text-xs">${escapeHtml(r.delivery_at.replace('T',' '))}</span>`
          : `<button class="flow-lm-deliver-now text-xs px-2 py-1 border rounded-lg bg-white hover:bg-gray-50" data-lane="${escapeAttr(r.laneKey)}" data-cont="${escapeAttr(r.key)}">Delivered (now)</button>`;
      ...
      detail.innerHTML = [
        ...
        `<div class="mt-3 rounded-xl border p-3">
          ...
          ${table(['Supplier', 'Zendesk', 'Freight', 'Container / AWB', 'Size', 'Vessel', 'Delivered', 'Status'], rows)}
        </div>`,
        editor,
      ].join('');
  ...
  function lastMileEditor(ws, tz, r) {
    ...
    return `
      <div class="mt-3 rounded-xl border p-3">
        ...
        <div class="mt-3">
          ${delivery
            ? `<div class="text-sm text-gray-700">Delivered at <b>${escapeHtml(delivery.replace('T',' '))}</b></div>`
            : `<div class="text-sm text-gray-700">Mark this container as delivered using the button below.</div>`}
        </div>
        ...
        <div class="flex items-center justify-between mt-3">
          ...
          <div class="flex items-center gap-2">
            ${delivery ? '' : `<button id="flow-lm-deliver-now" data-cont="${escapeAttr(r.key)}" data-lane="${escapeAttr(r.laneKey)}" class="px-3 py-1.5 rounded-lg text-sm border bg-emerald-50 hover:bg-emerald-100">Delivered (now)</button>`}
            <button id="flow-lm-save-note" data-cont="${escapeAttr(r.key)}" data-lane="${escapeAttr(r.laneKey)}" class="px-3 py-1.5 rounded-lg text-sm border bg-white hover:bg-gray-50">Save note</button>
          </div>
        </div>
      </div>
    `;
  ...
  function wireLastMileDetail(ws, selectedKey) {
    ...
    // Only supplier row buttons should change selection (exclude Delivered buttons)
    detail.querySelectorAll('[data-cont]:not(.flow-lm-deliver-now):not(#flow-lm-deliver-now)').forEach(btn => {
      if (btn.dataset.bound) return;
      btn.dataset.bound = '1';
    ...
    const patchContainer = (laneKey, contKey, patch) => {
      if (!laneKey || !contKey) return false;
    ...
    // Row-level Delivered (now)
    detail.querySelectorAll('.flow-lm-deliver-now').forEach(btn => {
      if (btn.dataset.bound) return;
      btn.dataset.bound = '1';
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const contKey = btn.getAttribute('data-cont') || '';
        const laneKey = btn.getAttribute('data-lane') || '';
        const ok = patchContainer(laneKey, contKey, {
          delivery_at: new Date().toISOString(),
          pod_received: true,
        });
        const msg = detail.querySelector('#flow-lm-save-msg');
        if (msg) msg.textContent = ok ? 'Delivered' : 'Could not update record';
        setTimeout(() => refresh(), 10);
      });
    });

    // Editor Delivered (now)
    const deliverBtn = detail.querySelector('#flow-lm-deliver-now');
    if (deliverBtn && !deliverBtn.dataset.bound) {
      deliverBtn.dataset.bound = '1';
      deliverBtn.addEventListener('click', () => {
        const contKey = deliverBtn.getAttribute('data-cont') || selectedKey;
        const laneKey = deliverBtn.getAttribute('data-lane') || '';
        const ok = patchContainer(laneKey, contKey, {
          delivery_at: new Date().toISOString(),
          pod_received: true,
        });
    ...
    // Editor Save note
    const saveNoteBtn = detail.querySelector('#flow-lm-save-note');
    if (saveNoteBtn && !saveNoteBtn.dataset.bound) {
      saveNoteBtn.dataset.bound = '1';
      saveNoteBtn.addEventListener('click', () => {
        const contKey = saveNoteBtn.getAttribute('data-cont') || selectedKey;
        const laneKey = saveNoteBtn.getAttribute('data-lane') || '';
        const note = detail.querySelector('#flow-lm-note')?.value || '';
        const ok = patchContainer(laneKey, contKey, { last_mile_note: String(note || '') });
        const msg = detail.querySelector('#flow-lm-save-msg');
        if (msg) msg.textContent = ok ? 'Saved' : 'Could not update record';
        setTimeout(() => refresh(), 10);
      });
    }
  }
