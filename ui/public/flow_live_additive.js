...
(function () {
  ...
  function computeReceivingStatus(ws, tz, planRows, receivingRows) {
    ...
    for (const r of asArray(planRows)) {
      ...
      if (!sup) continue;
      if (!poSupplierMap.has(po)) poSupplierMap.set(po, sup);
    }
  ...
  function injectSkeleton(page) {
    ...
    page.innerHTML = `
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <!-- Journey / nodes tile -->
        <div class="rounded-2xl border bg-white shadow-sm p-3 xl:col-span-2">
          <div class="flex items-center justify-between mb-2">
        ...
        <!-- Insights tile (populates when a node is selected) -->
        <div class="rounded-2xl border bg-white shadow-sm p-3 min-h-[320px]">
          ...
          <div class="text-xs text-gray-500 mb-2">Click a node to view details</div>
          <div id="flow-insights" class="mt-2"></div>
        </div>
      </div>

      <!-- Detail tile moved to bottom (full width) -->
      <div class="rounded-2xl border bg-white shadow-sm p-3 min-h-[320px] mt-3">
...
function renderTopNodes(ws, tz, receiving, vas, manual) {
  ...
  function renderInsights(ws, tz, receiving, vas, manual) {
    const box = document.getElementById('flow-insights');
    if (!box) return;

    const selNode = UI.selection?.node || null;

    const icon = (n) => {
      switch (n) {
        case 'milk': return '🚚';
        case 'receiving': return '✉️';
        case 'vas': return '✨';
        case 'intl': return '🛃';
        case 'lastmile': return '📍';
        default: return 'ℹ️';
      }
    };
    ...
    const titleFor = (n) => ({
      ...
      receiving: 'Receiving',
      vas: 'VAS applied',
      intl: 'Transit & Clearing',
      lastmile: 'Last Mile'
    }[n] || '');

    const levelFor = (n) => {
      if (n === 'milk') return 'upcoming';
      if (n === 'receiving') return receiving.status;
      if (n === 'vas') return vas.status;
      if (n === 'intl') return manual.intl.status;
      if (n === 'lastmile') return manual.lastMile.status;
      return 'on_track';
    };

    const level = selNode ? levelFor(selNode) : null;

    const pillText = (lvl) => {
      if (lvl === 'upcoming') return 'Upcoming';
      if (lvl === 'at_risk') return 'At‑Risk';
      if (lvl === 'delayed') return 'Delayed';
      return 'On Track';
    };
    ...
    const top = selNode ? `
      <div class="flex items-start justify-between gap-3">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full border bg-white flex items-center justify-center text-lg">${icon(selNode)}</div>
          <div>
            <div class="text-sm font-semibold text-gray-800">${titleFor(selNode)}</div>
            <div class="text-xs text-gray-500">Week of ${fmtDate(ws.start)} — ${fmtDate(ws.end)}</div>
          </div>
        </div>
        <span class="text-xs px-2 py-0.5 rounded-full border ${pill(level)}">${pillText(level)}</span>
      </div>
    ` : `
      <div class="text-sm text-gray-600">Select a node to view details.</div>
    `;

    // ---- Receiving: show reason + supplier exceptions (if data present) ----
    let body = '';
    if (selNode === 'receiving') {
      const late = Number(receiving.latePOs || 0);
      const planned = Number(receiving.plannedPOs || 0);
      const recd = Number(receiving.receivedPOs || 0);

      // Build PO->supplier lookup when possible (from receiving supplier rows)
      const poToSupplier = new Map();
      (receiving.poSupplierMap || []).forEach(x => { if (x?.po && x?.supplier) poToSupplier.set(String(x.po), String(x.supplier)); });
      ...
      // Aggregate metrics by supplier if row fields exist
      const agg = new Map();
      const bump = (supplier, key, inc) => {
        if (!supplier) return;
        if (!agg.has(supplier)) agg.set(supplier, { late: 0, damaged: 0, non: 0 });
        agg.get(supplier)[key] += inc;
      };

      // Late POs by supplier if we can map
      (receiving.latePOList || []).forEach(po => bump(poToSupplier.get(String(po)) || 'Unknown supplier', 'late', 1));

      // Damaged / Non-compliant by supplier if provided on supplier rows
      (receiving.suppliers || []).forEach(r => {
        const s = r.supplier || r.Supplier || r.name;
        bump(s, 'damaged', safeNum(r.damaged_cartons ?? r.damagedCartons ?? r.cartonsDamaged ?? r.damaged ?? 0));
        bump(s, 'non', safeNum(r.non_compliant ?? r.nonCompliant ?? r.cartonsNonCompliant ?? r.noncompliant ?? 0));
      });

      const topN = (key) => Array.from(agg.entries())
        .map(([supplier, v]) => ({ supplier, val: v[key] }))
        .filter(x => x.val > 0)
        ...
        if (!rows.length) {
          return `
            <div class="mt-3">
              ...
              <div class="text-xs text-gray-500 mt-1">No data available yet.</div>
            </div>
          `;
        }
        return `
          <div class="mt-3">
            ...
            <div class="mt-1 space-y-1">
              ${rows.map(x => `<div class="text-xs text-gray-700 flex items-center justify-between gap-2"><span class="truncate">${esc(x.supplier)}</span><span class="tabular-nums text-gray-500">${x.val}</span></div>`).join('')}
            </div>
          </div>
        `;
      };
      ...
      body = `
        <div class="mt-3 text-sm text-gray-700">
          <div class="flex flex-wrap gap-x-4 gap-y-1">
            <span><span class="text-gray-500">Planned:</span> <span class="tabular-nums">${planned}</span></span>
            <span><span class="text-gray-500">Received:</span> <span class="tabular-nums">${recd}</span></span>
            <span><span class="text-gray-500">Late:</span> <span class="tabular-nums">${late}</span></span>
          </div>
          ${late ? `<div class="mt-2 text-xs text-gray-600">Reason: <span class="font-medium">${late} POs were received after the baseline cut‑off.</span></div>` : ''}
        </div>
        ${block('Exceptions • Most delayed (by supplier)', topN('late'))}
        ${block('Exceptions • Most damaged (by supplier)', topN('damaged'))}
        ${block('Exceptions • Most non‑compliant (by supplier)', topN('non'))}
      `;
    }

    box.innerHTML = `
      <div class="text-sm font-semibold text-gray-700">Insights</div>
      <div class="text-xs text-gray-500">Context for the selected node</div>
  ...
  async function refresh() {
    ...
    // Cache raw rows for drill-down insights (used by the right-side tile)
    UI.cache = { planRows, receivingRows, records };
